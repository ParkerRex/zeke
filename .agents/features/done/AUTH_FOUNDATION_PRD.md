...
- Goal: deliver an incremental plan that hardens authentication/authorization and prepares the API/query layer for the new schema without breaking the existing engine ingestion.

## 2. Objectives
1. Provide a deterministic auth/session layer across the dashboard (`apps/dashboard`) using Supabase + Drizzle.
2. Bridge existing engine ingestion (`apps/engine`) with the upcoming schema changes by documenting required adjustments.
...
- We will not rewrite the engine task queue; only note schema-dependent changes.
...
- Map current engine writes to new tables:
...
- Create a migration checklist so engine deployment happens after Supabase migrations run.
...
2. **Data Layer Migration PRD**: Cover Drizzle schema changes, migrations, engine updates, and cached query implementations.
...
4. **Engine Sync**
   - Update `apps/engine/src/db.ts` functions to call new tables.
...
5. **Testing & Rollout**
   ...
   - Stage migrations in a Supabase branch; deploy engine with feature flag.
...
- [ ] Document required engine adjustments so login and team selection remain compatible during schema migration.
...
- Engine pipeline: `apps/engine/src/tasks` & `apps/engine/src/db.ts`

## 9. Risks & Mitigations
- **Migration mismatch**: Engine might write to old tables. Mitigation: feature flag new schema and deploy engine after migration cutover with smoke tests.
...
3. How will we backfill existing stories/highlights into the new schema (migration script vs engine replay)?

## 11. Acceptance Criteria
- A shared session helper exists and is consumed by server actions & API routes.
- Drizzle schema/migrations align with the ER diagram, and the engine code path compiles against them.
