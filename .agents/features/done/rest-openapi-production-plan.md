...
## Status
- ✅ Zeke scaffolding removed in favor of Zeke-specific routers.
- ✅ Shared context/middleware unified with TRPC (Supabase auth + team resolution).
- ✅ REST endpoints implemented for assistant, stories, highlights, search, tags, teams, and users with OpenAPI docs at `/api/openapi.json`.
## Objective
Deliver a minimal, well-typed REST surface that mirrors our TRPC contracts, emits an accurate OpenAPI spec, and aligns with Zeke's actual product surfaces (stories, highlights, assistant, search, team, tags, users). The goal is to remove the Zeke scaffolding that was copied in, replace it with Zeke-aware handlers, and keep REST thin by leaning on existing schemas and query helpers.

## Current State Findings
- **Zeke artifacts everywhere.** Routers such as `bank-accounts.ts`, `invoices.ts`, `transactions.ts`, and OAuth flows reference `@zeke/*` packages, nonexistent schemas, and domain concepts we do not ship. Several schema imports (e.g. `@api/schemas/customers`) do not exist, so the code does not even type-check.
- **Auth pipeline mismatch.** `middleware/auth.ts` expects Zeke API keys and caches, while TRPC already relies on Supabase JWTs + `withTeamPermission`. REST and TRPC contexts are diverging.
...
- **Router index exposes the wrong surface.** `routers/index.ts` wires up the Zeke suite and omits anything for stories/highlights/assistant.
...
- Rewrite `middleware/auth.ts` to exclusively use Supabase JWTs for now (mirroring `verifyAccessToken`); stub API-key support as a follow-up instead of shipping broken Zeke logic.
...
- Update `withRequiredScope` messaging to drop Zeke terminology and support defaulting to read-only presets.
...
| --- | --- | --- |
| `bank-accounts.ts`, `documents.ts`, `invoices.ts`, `reports.ts`, `transactions.ts`, `tracker-entries.ts`, `tracker-projects.ts`, `notifications.ts`, `inbox.ts`, `oauth.ts` | Zeke-only; broken imports | Delete entirely. Document removal in changelog so future diffs show the intentional cut. |
...
| `teams.ts` | Closer to reality but still uses Zeke copy describing "workspace" | Update schema references, include `setActive` endpoint mirroring TRPC mutation, align success codes, and gate by `teams.read`/`teams.manage` scopes. |
...
- Remove dead schema files copied from Zeke once their routers go away (e.g. delete `apps/api/src/schemas/bank-accounts.ts` if added by mistake). If a schema is still valuable for future work, move it into `docs/backlog` with a note instead of shipping unused code.
