...
export const globalSearchSchema = z
  .object({
    searchTerm: z.string().optional().openapi({
      description:
        "Research query to discover insights across ingested knowledge sources.",
      example: "market expansion strategy",
    }),
    language: z.string().optional().openapi({
      description:
        "Language for contextual understanding and cited output generation.",
      example: "en",
    }),
    ...
    itemsPerTableLimit: z.coerce.number().min(1).max(100).default(5).openapi({
      description:
        "Discovery depth per entity type for comprehensive research coverage.",
      example: 5,
    }),
    relevanceThreshold: z.coerce.number().min(0).max(1).default(0.01).openapi({
      description:
        "Minimum confidence score for insight inclusion in research outcomes.",
      example: 0.01,
    }),
  })
  .openapi({
...
export const searchResponseSchema = z
  .array(
    z.object({
      id: z.string().openapi({
        description:
          "Citation reference for tracking insight provenance in published outputs.",
      ...
      relevance: z.number().openapi({
        description:
          "Goal-alignment score for prioritizing insights during triage.",
        example: 0.92,
      }),
      created_at: z.string().openapi({
        description:
          "Temporal context for research freshness and trend analysis.",
  ...
  .openapi({
    description:
      "Discovered insights triaged for goal-aware application and publishing.",
    example: [
      {
...
export const globalSearchInputSchema = globalSearchSchema;
export const semanticSearchInputSchema = globalSearchSchema;
export const searchResultSchema = searchResponseSchema;
