-- Migration: Add chat tables and drop legacy thread tables
-- Description: Implements new chat persistence for the assistant feature, replacing legacy thread system
-- Date: 2025-09-25

-- Step 1: Drop legacy thread tables and their foreign keys
...
-- Drop any orphaned foreign key constraints
ALTER TABLE public.highlights DROP CONSTRAINT IF EXISTS highlights_assistant_thread_id_fkey;
ALTER TABLE public.activities DROP CONSTRAINT IF EXISTS activities_thread_id_assistant_threads_id_fk;

-- Step 2: Create new chat tables for the assistant
...
-- Step 4: Add foreign key constraints for referential integrity
...
-- Step 5: Grant appropriate permissions (adjust as needed for your roles)
-- Assuming standard Supabase roles: anon, authenticated, service_role
GRANT SELECT ON "chats", "chat_messages", "chat_feedback" TO authenticated;
GRANT INSERT, UPDATE, DELETE ON "chats", "chat_messages", "chat_feedback" TO authenticated;

-- Add RLS policies (to be implemented based on specific requirements)
