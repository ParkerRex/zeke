...
echo -e "${BLUE}🛑 Stopping app development services...${NC}"
...
# Stop Next.js app
stop_port 3000 "Next.js App"

# Stop worker containers
echo -e "${YELLOW}🔄 Stopping worker containers...${NC}"
if command -v docker >/dev/null 2>&1; then
    local worker_containers=$(docker ps -q --filter "name=zeke-worker-local-8082" 2>/dev/null || true)
...
echo -e "${GREEN}🎉 App development services stopped!${NC}"
