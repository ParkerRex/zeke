...
import { getTeamSubscriptionAction } from "@/actions/billing/get-team-subscription-action";
...
export default async function AccountPage() {
  ...
  const [productsResult, subscriptionResult] = await Promise.all([
    getBillingProductsAction({}),
    getTeamSubscriptionAction({}),
  ]);

  if (productsResult.serverError) {
    throw new Error(productsResult.serverError);
  }

  const products = productsResult.data ?? [];
  const subscription = subscriptionResult.data ?? null;

  const ignoredErrors = ["User is not associated with a team"];
  if (
    subscriptionResult.serverError &&
    !ignoredErrors.includes(subscriptionResult.serverError)
  ) {
    throw new Error(subscriptionResult.serverError);
  }

  // Resolve current product/price
  const userProduct = subscription
    ? products.find((product) =>
        product.prices.some((price) => price.id === subscription.priceId),
      )
    : undefined;
