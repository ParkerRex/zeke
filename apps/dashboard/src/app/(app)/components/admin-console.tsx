"use client";
import { createSupabaseBrowserClient } from "@zeke/auth/clients/browser";
...
import ForecastPlayground from "./forecast-playground";
...
export default function AdminConsole() {
	...
	const [qKind, setQKind] = useState<
		"rss" | "podcast" | "youtube_channel" | "youtube_search"
	>("rss");
	// Overview/Jobs state
		...
		async function tick() {
			try {
				const [s, r] = await Promise.all([
					fetch("/api/pipeline/status", { cache: "no-store" }).then((x) =>
						x.json(),
					),
					fetch("/api/pipeline/recent", { cache: "no-store" }).then((x) =>
						x.json(),
					),
				]);
				if (mounted) {
	...
	async function doPreview(id: string) {
		...
		try {
			const res = await fetch(`/api/admin/sources/${id}/preview?limit=10`, {
				cache: "no-store",
			});
			const json = await res.json();
			if (json?.ok) {
	...
	return (
		<div className="mx-auto max-w-7xl p-6">
			<h1 className="font-semibold text-xl tracking-tight">Admin Console</h1>
			<p className="mt-1 text-[13px] text-muted-foreground leading-snug">
				Manage sources, monitor jobs, estimate costs.
			</p>
			...
			<Tabs className="mt-4" onValueChange={setTab} value={tab}>
				...
				<TabsContent className="mt-4 space-y-6" value="sources">
					<Card>
						<CardHeader>
							<CardTitle>Add Source</CardTitle>
							<CardDescription>
								Paste any URL (YouTube/channel, RSS feed) or enter a name.
							</CardDescription>
						</CardHeader>
						<CardContent>
							<div className="flex flex-col gap-3 sm:flex-row">
								...
								<Button disabled={loading} onClick={upsertSource}>
									Add
								</Button>
							</div>
						</CardContent>
					...
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Paste one or more URLs (one per line). We’ll detect type and
								ingest once.
							</CardDescription>
						</CardHeader>
						<CardContent>
							<div className="flex flex-col gap-3">
								<textarea
									className="min-h-[140px] w-full rounded-md border bg-white p-3 text-sm"
								...
								<div className="flex gap-2">
									...
									{oneoffResult && (
										<div className="self-center text-muted-foreground text-xs">
											{oneoffResult.filter((r: any) => r.ok).length} enqueued,{" "}
											{oneoffResult.filter((r: any) => !r.ok).length} skipped
										</div>
									)}
								</div>
					...
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Active sources by kind; pause/resume and edit.
							</CardDescription>
						</CardHeader>
						<CardContent>
											...
											<tr className="border-b" key={s.id}>
													...
													{(() => {
														...
														return (
															<span className="text-muted-foreground text-xs">
																...
																{m.contents_total ?? 0} / St{" "}
																{m.stories_total ?? 0}
																{typeof m.stories_24h === "number" &&
																	m.stories_24h > 0 && (
												...
												<td className="space-x-2 py-2 pr-3">
													...
													<span className="inline-flex items-center gap-1">
														...
														<Button
															disabled={previewLoading && preview?.id === s.id}
															onClick={() => doPreview(s.id)}
															size="sm"
															variant="outline"
														>
															Preview
					...
					{preview && (
						<Card>
							<CardHeader>
								...
								<CardDescription>
									Top candidates to ingest. No writes performed.
								</CardDescription>
							</CardHeader>
							<CardContent>
								{preview.quota && (
									<div className="mb-3 text-muted-foreground text-xs">
										Quota used: {preview.quota.used} • Remaining:{" "}
									...
									<ul className="space-y-1.5">
										{preview.items.map((it: any) => (
											<li
												...
												key={it.external_id || it.url}
											>
												<div className="truncate leading-tight">
													{it.title || it.url}
												</div>
												<div className="mt-0.5 truncate text-[12px] text-muted-foreground leading-snug">
													{it.url}
												</div>
											</li>
										))}
				...
				<TabsContent className="mt-4 space-y-6" value="overview">
					<div className="flex items-end justify-between gap-4">
						<div>
							...
							<p className="mt-1 text-[13px] text-muted-foreground leading-snug">
								Live status updates every 2s. Use actions to manually kick off
								ingest.
							</p>
						</div>
						<div className="flex gap-2">
							<Button disabled={busy} onClick={() => trigger("rss")}>
								Trigger RSS Ingest
							</Button>
							<Button
								disabled={busy}
								onClick={() => trigger("youtube")}
								variant="secondary"
							>
					...
					<div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
						<Card className="lg:col-span-2">
							<CardHeader>
								...
								<CardDescription>
									High-level counts across the ingestion funnel.
								</CardDescription>
							</CardHeader>
							<CardContent>
								<div className="grid grid-cols-2 gap-3 text-sm md:grid-cols-4">
									<div className="border px-3 py-2.5">
										...
										<div className="mt-0.5 font-semibold text-lg">
											{status?.worker?.sources?.sources_rss ?? "–"}
										</div>
									</div>
									<div className="border px-3 py-2.5">
										<div className="text-[13px] text-muted-foreground">
										...
										<div className="mt-0.5 font-semibold text-lg">
											{status?.worker?.raw?.raw_total ??
												status?.counts?.raw_items ??
												"–"}
										</div>
									</div>
									<div className="border px-3 py-2.5">
										...
										<div className="mt-0.5 font-semibold text-lg">
											{status?.worker?.contents?.contents_total ??
												status?.counts?.contents ??
												"–"}
										</div>
									</div>
									<div className="border px-3 py-2.5">
					...
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Top job/state counts from the worker.
							</CardDescription>
						</CardHeader>
						<CardContent>
							{(() => {
					...
					<div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
						<Card>
							<CardHeader>
								...
								<CardDescription>
									Newest items discovered from sources.
								</CardDescription>
							</CardHeader>
							<CardContent>
								<div className="max-h-80 overflow-auto text-sm">
									<ul className="space-y-1.5">
										{(recent?.raw_items ?? []).map((r: any) => (
											<li className="border px-3 py-2" key={r.id}>
												<div className="truncate leading-tight">
													{r.title ?? r.url ?? r.id}
												</div>
												<div className="mt-0.5 text-[12px] text-muted-foreground leading-snug">
						...
						<Card>
							<CardHeader>
								...
								<CardDescription>
									Fetched and parsed contents ready for analysis.
								</CardDescription>
							</CardHeader>
							<CardContent>
								<div className="max-h-80 overflow-auto text-sm">
									<ul className="space-y-1.5">
										{(recent?.contents ?? []).map((c: any) => (
											<li className="border px-3 py-2" key={c.id}>
												<div className="truncate leading-tight">
													{c.html_url ?? c.id}
												</div>
												<div className="mt-0.5 text-[12px] text-muted-foreground leading-snug">
													{new Date(c.created_at).toLocaleString()} • raw{" "}
													{String(c.raw_item_id).slice(0, 8)}
												</div>
											</li>
										))}
						...
						<Card>
							<CardHeader>
								...
								<CardDescription>
									Stories created or updated recently.
								</CardDescription>
							</CardHeader>
							<CardContent>
								<div className="max-h-80 overflow-auto text-sm">
									<ul className="space-y-1.5">
										{(recent?.stories ?? []).map((s: any) => (
											<li className="border px-3 py-2" key={s.id}>
												<div className="truncate leading-tight">
													{s.title ?? s.canonical_url ?? s.primary_url ?? s.id}
												</div>
												<div className="mt-0.5 text-[12px] text-muted-foreground leading-snug">
				...
				<TabsContent className="mt-4" value="jobs">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Job queue is summarized in Overview above.
							</CardDescription>
						</CardHeader>
						<CardContent className="text-muted-foreground text-sm">
							Use Overview for live queue + recent lists.
						</CardContent>
					</Card>
				</TabsContent>
			</Tabs>
...
function QuotaCard() {
	...
	useEffect(() => {
		...
		const channel = supabase
			...
			.on(
				"postgres_changes",
				{ event: "*", schema: "public", table: "platform_quota" },
				(payload: any) => {
					const row = (payload.new ?? payload.record) as any;
					setQuota((prev) => {
				...
				{quota.map((q) => (
					...
					>
						<div className="truncate pr-4">
							<span className="font-medium">{q.provider}</span>
						</div>
						<div className="text-right">
							<div>
								Used {q.used ?? "–"} / {q.quota_limit ?? "–"}
							</div>
							<div className="text-muted-foreground text-xs">
								Remaining {q.remaining ?? "–"} • Reset{" "}
								{q.reset_at ? new Date(q.reset_at).toLocaleString() : "–"}
							</div>
						</div>
					</div>
				))}
