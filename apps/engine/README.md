...
Background engine service for Zeke using pg-boss. Handles ingest (RSS/YouTube), extraction (audio → transcript → content), and analysis (LLM overlays/embeddings).

**🎉 New Modular Architecture**: The engine has been restructured into a clean, beginner-friendly modular system. See [ARCHITECTURE.md](ARCHITECTURE.md) for details.
...
4. **Start engine**:
   ...
   bash scripts/deploy-local-engine.sh
...
```bash
DATABASE_URL_POOLER="postgresql://engine.xxx:xxx@aws-1-us-central1.pooler.supabase.com:5432/postgres?sslmode=no-verify"
BOSS_SCHEMA="pgboss"
BOSS_MIGRATE="false"
OPENAI_API_KEY="your-openai-key"
YOUTUBE_API_KEY="your-youtube-key"
...
| Script                                | Description                                    |
| ------------------------------------- | ---------------------------------------------- |
| `bash scripts/deploy-local-engine.sh` | Start engine in Docker (yt-dlp/ffmpeg/Whisper) |
...
- **Regional Matching**: Ensure engine and database in same region

## 📊 Monitoring

### Health Checks

- Engine exposes health endpoint on `/health`
