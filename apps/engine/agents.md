...
This guide orients coding agents working inside `engine/` so changes stay reliable, efficient, and secure.

**Scope:** Background pipeline (ingest → extract → analyze) using PostgreSQL, `pg-boss`, and external APIs. Do not use the Supabase JS SDK in the engine.

## Project Overview

- **Purpose:** Ingest sources (RSS, YouTube), fetch and extract content, generate analysis/embeddings, and manage jobs.
- **Key Modules:**
  - `src/engine.ts`: HTTP health/debug endpoints, `pg-boss` queues, schedulers, engines
...
- **Local (Docker, full deps):** `bash scripts/deploy-local-engine.sh`
...
- **DB access (critical):**
  - Use the shared `pg` Pool from `src/db.ts`. Do not import or use the Supabase JS SDK in the engine.
...
1. **Define queue:** In `src/engine.ts`, ensure `boss.createQueue('my:queue')` and schedule if recurring.
2. **Implement engine:** `await boss.work('my:queue', opts, async (jobs) => { ... })` using structured logging and try/catch per job.
...
5. **Test:** Use local `.env.development`, run `pnpm run dev`, and verify via `/debug/status` and scripts in `engine/scripts/`.
...
By following these conventions—especially direct `pg` usage instead of the Supabase SDK—the engine remains efficient, portable, and production‑safe.
...
## Tasks Catalog (engine/src/tasks)
