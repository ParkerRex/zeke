...
echo -e "${BLUE}🧪 Running Engine Integration Tests...${NC}"
...
echo -e "${BLUE}📍 Testing against: ${ENGINE_URL}${NC}"

# Test 1: Engine Health Check
echo -e "\n${YELLOW}1️⃣ Testing engine health...${NC}"

if curl -f -s "${ENGINE_URL}/healthz" >/dev/null; then
    echo -e "${GREEN}✅ Engine health check passed${NC}"
else
    echo -e "${RED}❌ Engine health check failed - is engine running?${NC}"
    echo -e "${YELLOW}💡 Try: npm run dev:engine${NC}"
    exit 1
fi
...
STATUS_RESPONSE=$(curl -f -s "${ENGINE_URL}/debug/status" || echo "failed")
...
RSS_RESPONSE=$(curl -f -s -X POST "${ENGINE_URL}/debug/ingest-now" || echo "failed")
...
if [ -n "${YOUTUBE_API_KEY:-}" ] && [[ "${YOUTUBE_API_KEY}" != *"test"* ]]; then
    YT_RESPONSE=$(curl -f -s -X POST "${ENGINE_URL}/debug/ingest-youtube" || echo "failed")
...
ONEOFF_RESPONSE=$(curl -f -s -X POST "${ENGINE_URL}/debug/ingest-oneoff" \
...
STATUS_RESPONSE_2=$(curl -f -s "${ENGINE_URL}/debug/status" || echo "failed")
...
INVALID_RESPONSE=$(curl -s -X POST "${ENGINE_URL}/debug/ingest-source?sourceId=invalid-source-id" || echo "failed")
...
MISSING_RESPONSE=$(curl -s -X POST "${ENGINE_URL}/debug/ingest-source" || echo "failed")
...
if [ -f "dist/engine.js" ] && [ -f "src/core/engine-service.js" ]; then
...
echo -e "   ✅ Engine health and status"
...
echo -e "\n${GREEN}🚀 Engine integration tests passed!${NC}"
echo -e "${BLUE}💡 The engine is ready for production use.${NC}"
