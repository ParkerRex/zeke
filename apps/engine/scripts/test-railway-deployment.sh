...
if [ ! -f "dist/engine.js" ]; then
    echo -e "${RED}❌ Build failed - dist/engine.js not found${NC}"
    exit 1
fi
...
if command -v docker &> /dev/null; then
    ...
    if docker build -t zeke-engine-test . --quiet; then
        echo -e "${GREEN}✅ Docker build successful${NC}"
        
        # Clean up test image
        docker rmi zeke-engine-test --force > /dev/null 2>&1 || true
    else
...
echo -e "${YELLOW}🔧 Starting engine service on port $TEST_PORT...${NC}"
...
if ! kill -0 $ENGINE_PID 2>/dev/null; then
    echo -e "${RED}❌ Engine process died during startup${NC}"
    exit 1
fi

echo -e "${GREEN}✅ Engine process started (PID: $ENGINE_PID)${NC}"
...
echo -e "${GREEN}✅ Engine process starts correctly${NC}"
echo -e "${GREEN}✅ Health check endpoint responds${NC}"

echo -e "${BLUE}🎯 Railway Deployment Readiness${NC}"
echo -e "${GREEN}✅ Your engine is ready for Railway deployment!${NC}"
echo ""
