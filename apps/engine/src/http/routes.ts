/**
 * HTTP Routes - All engine HTTP endpoints in one place
 *
 * This module handles all HTTP endpoints for the engine service.
...
/**
 * Sets up all HTTP routes for the engine service
 */
...
function setupDebugRoutes(
  ...
  // Trigger YouTube ingest (disabled)
  app.post("/debug/ingest-youtube", (_req, res) => {
  ...
  app.post("/debug/ingest-source", async (req, res) => {
    try {
      ...
      if (sourceKind === "rss" || sourceKind === "podcast") {
        ...
        return res
          .status(HTTP_BAD_REQUEST)
          .json({ ok: false, error: "youtube_ingest_disabled" });
      } else {
        return res
          .status(HTTP_BAD_REQUEST)
