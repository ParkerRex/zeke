...
export function HypeBar({ value, className }: HypeBarProps) {
  ...
  const getHypeColor = (hype: number) => {
    if (hype < 20) return 'bg-muted';
    if (hype < 40) return 'bg-yellow-400';
    if (hype < 60) return 'bg-orange-400';
    if (hype < 80) return 'bg-red-400';
    return 'bg-red-600';
  };

  const getHypeLabel = (hype: number) => {
    if (hype < 20) return 'Low';
    if (hype < 40) return 'Mild';
    if (hype < 60) return 'Moderate';
