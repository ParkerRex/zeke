...
Anywhere the app imports `@midday/supabase/*` (for example, `apps/dashboard/src/components/google-sign-in.tsx`) it receives a fully typed Supabase client.
...
- Application code imports from `@midday/db/queries` to keep SQL in one place and avoid duplicating schema knowledge.

## Type generation and CI/CD flow

- Supabase types are generated with the CLI: `pnpm --filter @midday/supabase db:generate`. The script in `packages/supabase/package.json` runs `supabase gen types … > src/types/db.ts`. Commit the updated file whenever the hosted Supabase schema changes.
