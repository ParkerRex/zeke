...
- Recent additions include `billing.ts` (Stripe products/subscriptions), `story-clusters.ts` (cluster overlays + transcripts), and `pipeline.ts` (ingestion counts/activity) so UI/server actions can pull rich data without raw SQL.
- Application code imports from `@zeke/db/queries` to keep SQL in one place and avoid duplicating schema knowledge.

## Mutations and compositions

- Mutations sit alongside their read counterparts for now (e.g. `stories.ts` exposes both list helpers and `insertStory`/`upsertStoryOverlay`). Each helper accepts the `Database` type from `connectDb()` and stays focused on a single responsibility until we split out a dedicated mutations surface.
- Composition helpers bundle multiple queries/mutations into the shapes the UI expects (Discover → Apply payloads, assistant threads, pipeline dashboards). Place them in `packages/db/src/compositions` so both server actions and workers can reuse the same builders.
- Keep input/output contracts close to the data layer by exporting types alongside each helper. When validation is required, colocate Zod schemas in a `validation` folder so API routes, safe actions, and background jobs stay in sync.
