...
async function fetchPlaybookSummaries(
	...
	let query = db
		.select({
			...
			stepCount: sql<number>`cast(count(distinct ${playbookSteps.id}) as int)`.
				as("stepCount"),
			highlightCount: sql<number>`cast(count(distinct ${playbookStepHighlights.id}) as int)`.
				as("highlightCount"),
		})
		.from(playbooks)
		.leftJoin(playbookTemplates, eq(playbooks.template_id, playbookTemplates.id))
...
async function loadStepHighlights(
	...
	const rows = await db
		...
		.innerJoin(highlights, eq(highlights.id, playbookStepHighlights.highlight_id))
		.where(inArray(playbookStepHighlights.playbook_step_id, stepIds))
		.orderBy(playbookStepHighlights.playbook_step_id, playbookStepHighlights.id);
	...
	return rows.map((row) => ({
		...
		templateStep: row.templateStepId
			? {
				...
				defaultPayload: (row.templateStepDefaultPayload ?? null) as
...
export async function getPlaybookTemplates(
	...
	const rows = await db
		.select({
			...
			stepCount: sql<number>`cast(count(${playbookTemplateSteps.id}) as int)`.
				as("stepCount"),
		})
	...
	return {
		...
		steps: stepRows.map((row) =>
			mapTemplateStepRow({
				...
				outputType: row.outputType,
				position: row.position,
				defaultPayload: (row.defaultPayload ?? null) as
					Record<string, unknown> | null,
			}),
		),
	};
...
export async function updatePlaybookStatus(
	db: Database,
	params: { playbookId: string; teamId: string; status: PlaybookStatus; publishedAt?: string | null },
	...
	await db
		...
		.where(and(eq(playbooks.id, params.playbookId), eq(playbooks.team_id, params.teamId)));
...
export async function attachHighlightToStep(
	...
	const highlightsByStep = await loadStepHighlights(db, [params.playbookStepId]);
