...
export async function getHighlightEngagement(
	...
	const playbookLinks = db
		...
		.innerJoin(playbookSteps, eq(playbookSteps.id, playbookStepHighlights.playbook_step_id))
	...
	const threadLinks = db
		...
		.innerJoin(assistantThreads, eq(assistantThreads.id, assistantThreadSources.thread_id))
