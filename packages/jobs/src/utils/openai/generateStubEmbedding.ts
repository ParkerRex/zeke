...
export async function generateStubEmbedding(story: EmbeddingInput): Promise<EmbeddingResult> {
	...
	for (let i = 0; i < content.length && i < EMBEDDING_DIMENSIONS; i++) {
		...
		embedding[i % EMBEDDING_DIMENSIONS] +=
			((charCode / CHAR_NORMALIZATION) - CONFIDENCE_DEFAULT) * EMBEDDING_NORMALIZATION_FACTOR;
	}
