...
export async function upsertUserSubscription({
  ...
  const subscription = await stripeAdmin.subscriptions.retrieve(subscriptionId, { expand: ['default_payment_method'] });
...
const copyBillingDetailsToCustomer = async (userId: string, paymentMethod: Stripe.PaymentMethod) => {
  ...
  if (typeof customer !== 'string') throw new Error('Customer id not found');
