...
export default function IndustryPanel() {
  ...
  const [sectorSlug, subSectorSlug] = Array.isArray(params?.slug) ? (params.slug as string[]) : [];
  ...
  const sectorMap = useMemo(() => {
    ...
    for (const n of DATA) {
      ...
      for (const c of n.children ?? []) bySlug.set(slugify(c), c);
    }
    return bySlug;
  }, []);

  const selectedSectorLabel = sectorSlug ? sectorMap.get(sectorSlug) : undefined;
  const selectedSubLabel = subSectorSlug ? sectorMap.get(subSectorSlug) : undefined;
          ...
          return (
            <div key={n.name}>
              <Link
                ...
                className='mb-2 block w-full rounded-md border bg-white px-3 py-2 text-left text-sm font-medium hover:border-gray-300 hover:bg-gray-50'
              >
                {n.name}
              </Link>
