...
import { BillingInterval, Price, ProductWithPrices } from '@/types/pricing';
...
export function PricingCard({
  ...
  const [billingInterval, setBillingInterval] = useState<BillingInterval>(price ? (price.interval as BillingInterval) : 'month');
    ...
    <WithSexyBorder className='w-full'>
      <div className='flex h-full w-full flex-col rounded-2xl border border-gray-200 bg-white p-6 lg:p-8 shadow-sm'>
        <div className='text-center'>
          ...
          <div className='mb-6 flex items-end justify-center gap-2 text-gray-900'>
            <span className='text-5xl font-extrabold tracking-tight'>
              {yearPrice && isBillingIntervalYearly ? '$' + yearPrice / 100 : monthPrice ? '$' + monthPrice / 100 : 'Custom'}
            </span>
        ...
        <div className='mx-auto w-full max-w-xs flex-1 space-y-3 py-2'>
          {metadata.researches === 'unlimited' ? <CheckItem text={`Unlimited research`} /> : <CheckItem text={`Up to ${metadata.researches} researches`} />}
        ...
        {createCheckoutAction && (
          <div className='mt-6'>
            {currentPrice && (
              <Button variant={buttonVariantMap[metadata.priceCardVariant]} className='w-full' onClick={() => createCheckoutAction({ price: currentPrice })}>
                Get Started
              </Button>
            )}
            {!currentPrice && (
...
function PricingSwitch({ onChange }: { onChange: (value: BillingInterval) => void }) {
  return (
    <Tabs defaultValue='month' className='flex items-center' onValueChange={(newBillingInterval) => onChange(newBillingInterval as BillingInterval)}>
