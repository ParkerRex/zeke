 ...
 */
export async function runAnalyzeStory(storyId) {
...
async function generateOpenAIAnalysis(story) {
    ...
    const truncatedText = story.text.length > maxContentLength ? story.text.substring(0, maxContentLength) + '...[truncated]' : story.text;
...
async function generateStubAnalysis(story) {
    ...
    const hasImportantKeywords = titleWords.some((word) => ['ai', 'artificial', 'intelligence', 'breakthrough', 'security', 'privacy', 'data'].includes(word));
    const domain = story.canonical_url ? new URL(story.canonical_url).hostname : 'unknown';
 ...
 */
async function generateEmbedding(story) {
    if (USE_OPENAI) {
 ...
 */
async function generateOpenAIEmbedding(story) {
    // Combine title and text for embedding
    const content = `${story.title || ''}\n\n${story.text}`;
    // Truncate if too long (text-embedding-3-small has ~8k token limit)
    const maxContentLength = 6000; // Conservative limit
    const truncatedContent = content.length > maxContentLength ? content.substring(0, maxContentLength) + '...[truncated]' : content;
    try {
 ...
 */
async function generateStubEmbedding(story) {
