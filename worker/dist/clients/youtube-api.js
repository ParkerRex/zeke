...
export class YouTubeAPIClient {
    ...
    constructor() {
        ...
        this.quotaLimit = Number.parseInt(process.env.YOUTUBE_QUOTA_LIMIT || "10000");
        this.quotaBuffer = Number.parseInt(process.env.YOUTUBE_RATE_LIMIT_BUFFER || "500");
        this.quotaResetHour = Number.parseInt(process.env.YOUTUBE_QUOTA_RESET_HOUR || "0");
    }
    /**
        ...
        try {
            ...
            for (const item of searchResponse.data?.items || []) {
                if (item.snippet && item.id?.channelId) {
                    ...
                    const uploadsPlaylistId = channelDetails.data.items?.[0]?.contentDetails?.relatedPlaylists
                        ?.uploads;
                    ...
                    if (item.snippet && item.id) {
                        allVideos.push({
                            ...
                            viewCount: item.statistics?.viewCount
                                ? Number.parseInt(item.statistics.viewCount)
                                : undefined,
