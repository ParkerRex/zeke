 ...
 */
export async function extractAudio(videoUrl, videoId) {
    ...
    try {
        ...
        const audioExtractionPromise = new Promise((resolve, reject) => {
        ...
        log('youtube_audio_extraction_error', {
            videoId,
            videoUrl,
            error: String(error),
        }, 'error');
...
export async function getVideoMetadata(videoId) {
    ...
    try {
        ...
        log('youtube_metadata_extraction_error', {
            videoId,
            error: String(error),
        }, 'error');
...
export async function checkYtDlpAvailability() {
    try {
        ...
        log('youtube_ytdlp_availability_error', {
            error: String(error),
        }, 'error');
        return false;
    }
 ...
 */
export async function getSupportedFormats(videoUrl) {
    try {
        const ytDlpArgs = [videoUrl, '--list-formats', '--no-playlist'];
        const formatsPromise = new Promise((resolve, reject) => {
            const ytDlp = spawn('yt-dlp', ytDlpArgs);
            let stdout = '';
            ytDlp.stdout.on('data', (data) => {
                stdout += data.toString();
        ...
        log('youtube_formats_check_error', {
            videoUrl,
            error: String(error),
        }, 'error');
        return [];
