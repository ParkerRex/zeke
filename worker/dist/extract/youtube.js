...
import { extractAudio } from './youtube-audio.js';
 ...
 */
export async function runYouTubeFetchAndExtract(jobData, boss) {
 ...
 */
function formatYouTubeContent(transcriptText, metadata, transcriptionResult) {
    const sections = [];
    ...
    if (metadata.description) {
        sections.push(`Description: ${metadata.description.substring(0, 500)}${metadata.description.length > 500 ? '...' : ''}`);
    }
 ...
 */
export async function runYouTubeQueuedExtract(jobData, boss) {
    const { rawItemIds, videoId, sourceKind } = jobData;
    const rows = await findRawItemsByIds(rawItemIds || []);
    for (const row of rows) {
        try {
            ...
            log('youtube_queued_extract_error', {
                comp: 'extract',
                raw_item_id: row.id,
                video_id: videoId,
                err: String(err),
