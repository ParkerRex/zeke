 ...
 */
export function createTempPath(videoId, extension) {
...
export async function ensureTempDirectory() {
    try {
        ...
        log('temp_directory_creation_error', {
            path: TEMP_BASE_DIR,
            error: String(error),
        }, 'error');
        throw error;
 ...
 */
export async function cleanupVideoTempFiles(videoId) {
    try {
        const sanitizedVideoId = videoId.replace(/[^a-zA-Z0-9_-]/g, '_');
 ...
 */
export async function checkTempDiskSpace(requiredBytes) {
    try {
        ...
        log('temp_disk_space_check_error', {
            error: String(error),
        }, 'warn');
 ...
 */
export async function createTempFile(videoId, extension, content) {
