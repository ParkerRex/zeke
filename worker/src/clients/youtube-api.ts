...
export interface QuotaTracker {
  ...
  reserveBuffer: number; // Units to keep in reserve
}
...
export class YouTubeAPIClient {
   ...
   */
  async checkQuotaStatus(currentUsage: number = 0): Promise<{ used: number; remaining: number; canProceed: boolean }> {
    const now = new Date();
  ...
  async searchVideos(
    ...
    duration: 'short' | 'medium' | 'long' | 'any' = 'any'
  ): Promise<YouTubeVideo[]> {
    try {
   ...
   */
  async withRetry<T>(operation: () => Promise<T>, maxRetries: number = 3, baseDelay: number = 1000): Promise<T> {
