...
import { YouTubeAPIClient, YouTubeVideo } from '../clients/youtube-api.js';
...
export interface FetchSearchVideosOptions {
  maxResults?: number;
  publishedAfter?: string; // ISO 8601 date
...
export class YouTubeFetcher {
  ...
  async fetchChannelVideos(
    ...
    try {
      ...
      log(
        'youtube_fetch_channel_videos_error',
        {
          uploadsPlaylistId,
          error: String(error),
        },
   ...
   */
  async searchVideos(query: string, options: FetchSearchVideosOptions = {}): Promise<YouTubeVideo[]> {
