...
async function testYouTubePipeline() {
  ...
  try {
    ...
    if (metadata.duration && metadata.duration > 360) {
      // 6 minutes
      console.log(
        "⚠️  Video is longer than 6 minutes, skipping audio extraction for this test"
      );
    ...
    console.log(
      `   - File size: ${(audioResult.fileSizeBytes / 1024 / 1024).toFixed(
        2
      )} MB`
    );
    ...
    console.log(
      `   - Text length: ${transcriptionResult.text.length} characters`
    );
    console.log(
      `   - Transcription time: ${transcriptionResult.transcriptionTimeMs}ms`
    );
    console.log(
      `   - Text preview: ${transcriptionResult.text.substring(0, 200)}...`
    );
    ...
    console.log(
      `   - Total processing time: ${
        audioResult.extractionTimeMs + transcriptionResult.transcriptionTimeMs
      }ms`
    );
    console.log(
      `   - Audio file size: ${(
